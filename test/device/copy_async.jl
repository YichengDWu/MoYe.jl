...
function copy_kernel(dest, src, blocklayout, threadlayout)
    smem = Shambles.SharedMemory(eltype(dest), cosize(blocklayout))
    cute_smem = CuTeArray(smem, blocklayout)

    cute_dest = CuTeArray(pointer(dest), @Layout((256, 32))) # bug: has to be static
    cute_src = CuTeArray(pointer(src), @Layout((256, 32)))
...
@cuda blocks=blocks threads=threads copy_kernel(a, b, blocklayout, threadlayout)
